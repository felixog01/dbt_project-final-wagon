with features as (
    select * from {{ ref('mart_features_communes') }}
),

stats as (
    select
        percentile_cont(production_kwh_kwc_an,     0.05) over() as p05_pvgis,
        percentile_cont(production_kwh_kwc_an,     0.95) over() as p95_pvgis,
        percentile_cont(irradiation_kwh_m2_an,     0.05) over() as p05_irrad,
        percentile_cont(irradiation_kwh_m2_an,     0.95) over() as p95_irrad,
        percentile_cont(wind_speed_moy_ms,         0.05) over() as p05_vent,
        percentile_cont(wind_speed_moy_ms,         0.95) over() as p95_vent,
        percentile_cont(productible_eolien_mwh_an, 0.25) over() as p25_productible,
        percentile_cont(productible_eolien_mwh_an, 0.75) over() as p75_productible,
        percentile_cont(pente_moy_deg,             0.05) over() as p05_pente,
        percentile_cont(pente_moy_deg,             0.95) over() as p95_pente,
        percentile_cont(cloud_cover_moy_pct,       0.05) over() as p05_cloud,
        percentile_cont(cloud_cover_moy_pct,       0.95) over() as p95_cloud,
        percentile_cont(wind_speed_max_ms,         0.05) over() as p05_rafale,
        percentile_cont(wind_speed_max_ms,         0.95) over() as p95_rafale
    from features
    limit 1
),

normalized as (
    select
        f.*,

        -- ── SOLAIRE ──
        least(1.0, greatest(0.0,
            safe_divide(f.production_kwh_kwc_an - s.p05_pvgis, s.p95_pvgis - s.p05_pvgis)
        ))                                                      as n_pvgis,

        least(1.0, greatest(0.0,
            safe_divide(f.irradiation_kwh_m2_an - s.p05_irrad, s.p95_irrad - s.p05_irrad)
        ))                                                      as n_irrad,

        -- NOUVEAU : exposition (orientation du terrain)
        case f.exposition_dominante
            when 'S'  then 1.00
            when 'SE' then 0.85
            when 'SO' then 0.85
            when 'E'  then 0.55
            when 'O'  then 0.55
            when 'NE' then 0.30
            when 'NO' then 0.30
            when 'N'  then 0.10
            else 0.50  -- Inconnu / plat = neutre
        end                                                     as n_exposition,

        -- NOUVEAU : couverture nuageuse (inversée : moins de nuages = mieux)
        least(1.0, greatest(0.0,
            1 - safe_divide(f.cloud_cover_moy_pct - s.p05_cloud, s.p95_cloud - s.p05_cloud)
        ))                                                      as n_cloud_inv,

        least(1.0, greatest(0.0,
            safe_divide(ln(f.surface_solaire_ha + 1), ln(10000))
        ))                                                      as n_surf_sol,

        least(1.0, greatest(0.0,
            1 - safe_divide(f.pente_moy_deg - s.p05_pente, s.p95_pente - s.p05_pente)
        ))                                                      as n_pente_inv,

        -- ── ÉOLIEN ──
        least(1.0, greatest(0.0,
            safe_divide(f.wind_speed_moy_ms - s.p05_vent, s.p95_vent - s.p05_vent)
        ))                                                      as n_vent,

        least(1.0, greatest(0.0,
            safe_divide(f.productible_eolien_mwh_an - s.p25_productible, s.p75_productible - s.p25_productible)
        ))                                                      as n_productible,

        -- NOUVEAU : relief éolien (courbe réaliste — modéré idéal, accidenté pénalisé)
        case
            when f.pente_moy_deg < 2   then 0.75   -- plat : bon mais pas d'accélération
            when f.pente_moy_deg < 5   then 0.90   -- léger relief : bon
            when f.pente_moy_deg < 10  then 1.00   -- relief modéré : IDÉAL (accélération vent)
            when f.pente_moy_deg < 15  then 0.70   -- accidenté : turbulences
            when f.pente_moy_deg < 20  then 0.40   -- très accidenté : accès difficile
            else 0.15                              -- montagne : quasi impraticable
        end                                                     as n_relief_eol,

        -- NOUVEAU : rafales max (garde-fou — rafales trop fortes = risque turbine)
        case
            when f.wind_speed_max_ms < 25 then 1.00   -- normal
            when f.wind_speed_max_ms < 35 then 0.80   -- soutenu
            when f.wind_speed_max_ms < 45 then 0.55   -- fort, vigilance
            else 0.30                                 -- tempétueux, risque
        end                                                     as n_rafale,

        least(1.0, greatest(0.0,
            safe_divide(ln(f.surface_eolien_ha + 1), ln(50000))
        ))                                                      as n_surf_eol,

        -- ── COMMUN ──
        case f.score_raccordement
            when 'Très favorable' then 0.70
            when 'Favorable'      then 0.50
            when 'Modéré'         then 0.25
            when 'Difficile'      then 0.05
            else 0.30
        end                                                     as n_raccordement,

        case
            when f.pct_territoire_protege >= 75 then 0.0
            when f.pct_territoire_protege >= 50 then 0.15
            when f.pct_territoire_protege >= 25 then 0.40
            when f.pct_territoire_protege >= 10 then 0.65
            when f.pct_territoire_protege >= 5  then 0.85
            else 1.0
        end                                                     as n_zones_inv,

        case f.fiabilite_vent
            when 'Très fiable'       then 0.80
            when 'Fiable'            then 0.55
            when 'Modérément fiable' then 0.30
            when 'Variable'          then 0.10
            else 0.05
        end                                                     as n_fiabilite_vent

    from features f
    cross join stats s
),

scored as (
    select
        *,
        -- SCORE SOLAIRE : PVGIS 30 + Irrad 22 + Exposition 12 + Zones 15 + Pente 8 + Nuages 3 + Surface 5 + Raccord 5
        round((
            0.30 * coalesce(n_pvgis,        0) +
            0.22 * coalesce(n_irrad,        0) +
            0.12 * coalesce(n_exposition,   0) +
            0.15 * coalesce(n_zones_inv,    0) +
            0.08 * coalesce(n_pente_inv,    0) +
            0.03 * coalesce(n_cloud_inv,    0) +
            0.05 * coalesce(n_surf_sol,     0) +
            0.05 * coalesce(n_raccordement, 0)
        ) * 100, 1)                                             as score_solaire,

        -- SCORE ÉOLIEN : Vent 35 + Productible 20 + Fiabilité 13 + Relief 12 + Zones 10 + Rafales 5 + Surface 5 + Raccord 5
        round((
            0.35 * coalesce(n_vent,             0) +
            0.20 * coalesce(n_productible,      0) +
            0.13 * coalesce(n_fiabilite_vent,   0) +
            0.12 * coalesce(n_relief_eol,       0) +
            0.10 * coalesce(n_zones_inv,        0) +
            0.05 * coalesce(n_rafale,           0) +
            0.05 * coalesce(n_surf_eol,         0) +
            0.05 * coalesce(n_raccordement,     0)
        ) * 100, 1)                                             as score_eolien

    from normalized
),

final as (
    select
        code_insee,
        nom_commune,
        latitude,
        longitude,
        code_departement,
        nom_departement,
        score_solaire,
        score_eolien,

        score_solaire >= 50 and surface_solaire_ha > 0          as eligible_solaire,
        score_eolien  >= 60 and surface_eolien_ha > 0           as eligible_eolien,

        case
            when score_solaire >= score_eolien then 'Solaire'
            else                                    'Éolien'
        end                                                     as technologie_dominante,

        case
            when score_solaire >= 70 then 'Top potentiel'
            when score_solaire >= 50 then 'Bon potentiel'
            else 'Non éligible'
        end                                                    as classe_solaire,

        case
            when score_eolien >= 75 then 'Top potentiel'
            when score_eolien >= 60 then 'Bon potentiel'
            else 'Non éligible'
        end                                                     as classe_eolien,

        -- Classe d'ensoleillement (équivalent solaire de classe_vent)
        -- Seuils calés sur les quartiles de l'irradiation française
        case
            when irradiation_kwh_m2_an >= 1582 then '☀️ Ensoleillement exceptionnel'
            when irradiation_kwh_m2_an >= 1472 then '🌤️ Bon ensoleillement'
            when irradiation_kwh_m2_an >= 1395 then '⛅ Ensoleillement modéré'
            else '☁️ Faible ensoleillement'
        end                                                     as classe_ensoleillement,


        -- Features clés
        nb_habitants,
        conso_moy_periode_mwh,
        conso_2017_mwh, conso_2018_mwh, conso_2019_mwh, conso_2020_mwh,
        conso_2021_mwh, conso_2022_mwh, conso_2023_mwh, conso_2024_mwh,
        evolution_conso_pct,
        production_kwh_kwc_an,
        irradiation_kwh_m2_an,
        wind_speed_moy_ms,
        productible_eolien_mwh_an,
        surface_solaire_ha,
        surface_eolien_ha,
        pct_territoire_protege,
        pente_moy_deg,
        altitude_moy_m,
        classe_pente_dominante,
        exposition_dominante,
        cloud_cover_moy_pct,
        wind_speed_max_ms,
        score_raccordement,
        taux_autonomie_pct,
        statut_enr,
        statut_autonomie,
        puissance_totale_mw,
        puissance_solaire_mw,
        puissance_eolien_mw,
        energie_totale_mwh_an,
        viable_solaire,
        viable_eolien,
        fiabilite_vent,
        classe_vent,
        has_natura2000,
        has_znieff,
        has_parc_national,
        has_pnr,
        has_reserve_naturelle,
        nb_installations_eol,
        nb_installations_pv,

        -- Production par année
        prod_sol_2017_mwh, prod_sol_2018_mwh, prod_sol_2019_mwh, prod_sol_2020_mwh,
        prod_sol_2021_mwh, prod_sol_2022_mwh, prod_sol_2023_mwh, prod_sol_2024_mwh,
        prod_eol_2017_mwh, prod_eol_2018_mwh, prod_eol_2019_mwh, prod_eol_2020_mwh,
        prod_eol_2021_mwh, prod_eol_2022_mwh, prod_eol_2023_mwh, prod_eol_2024_mwh,
        prod_tot_2017_mwh, prod_tot_2018_mwh, prod_tot_2019_mwh, prod_tot_2020_mwh,
        prod_tot_2021_mwh, prod_tot_2022_mwh, prod_tot_2023_mwh, prod_tot_2024_mwh,

        -- Prix terrain
        prix_terrain_median_eur_ha,
        prix_terrain_p25_eur_ha,
        prix_terrain_p75_eur_ha,

        -- Scores composantes (toutes, anciennes + nouvelles)
        round(coalesce(n_pvgis, 0) * 100, 1)             as score_composante_pvgis,
        round(coalesce(n_irrad, 0) * 100, 1)             as score_composante_irradiation,
        round(coalesce(n_exposition, 0) * 100, 1)        as score_composante_exposition,
        round(coalesce(n_cloud_inv, 0) * 100, 1)         as score_composante_nuages,
        round(coalesce(n_surf_sol, 0) * 100, 1)          as score_composante_surface_sol,
        round(coalesce(n_pente_inv, 0) * 100, 1)         as score_composante_pente,
        round(coalesce(n_vent, 0) * 100, 1)              as score_composante_vent,
        round(coalesce(n_productible, 0) * 100, 1)       as score_composante_productible,
        round(coalesce(n_relief_eol, 0) * 100, 1)        as score_composante_relief_eol,
        round(coalesce(n_rafale, 0) * 100, 1)            as score_composante_rafale,
        round(coalesce(n_surf_eol, 0) * 100, 1)          as score_composante_surface_eol,
        round(coalesce(n_zones_inv, 0) * 100, 1)         as score_composante_zones,
        round(coalesce(n_raccordement, 0) * 100, 1)      as score_composante_raccordement,
        round(coalesce(n_fiabilite_vent, 0) * 100, 1)    as score_composante_fiabilite_vent

    from scored
)

select * from final